...
async def start_safari(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Start safari for all accounts (iterates all user_* collections)."""
    user_id = update.effective_user.id
    ...
    started_count = 0
    # iterate over all user_* collections
    for col_name in db.list_collection_names():
        ...
        for account in accounts:
            ...
            if not acc_name or not session:
                continue
            try:
                res = await start_safari_for_account(uid, acc_name, session)
                # consider success if helper returns a positive/OK emoji or 'Started' substring
                if res and (res.startswith("✅") or "Started" in res):
                    started_count += 1
                # tiny throttle so we don't spam starting dozens at once
                await asyncio.sleep(0.15)
            except Exception as e:
                print(f"❌ Error starting {acc_name} (user {uid}): {e}")
                continue
...
async def stop_safari(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Stop safari for all accounts (iterates all user_* collections)."""
    user_id = update.effective_user.id
    if not is_owner(user_id):
        await update.message.reply_text("❌ Only the owner can use this command.")
        return
    ...
    for col_name in db.list_collection_names():
        ...
        for account in accounts:
            ...
            try:
                ...
                if res and (res.startswith("🛑") or "Stopped" in res):
                    stopped_count += 1
                await asyncio.sleep(0.05)
            except Exception as e:
                print(f"❌ Error stopping {acc_name} (user {uid}): {e}")
                continue
...
async def run_safari_hunt(user_id: int, account_name: str, client: TelegramClient, safari_bot):
    ...
    try:
        ...
        # Small wait before trying to enter
        ...
        while True:
            ...
            if "entry fee deducted" in text or "you are already in the" in text or "welcome" in text:
                entered = True
                await notify_safari_status(user_id, account_name, "✅ Entered Safari Zone")
                break
        ...
        # Start hunting
        await notify_safari_status(user_id, account_name, "🎯 Starting hunt...")
